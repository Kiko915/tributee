    ...
    <div class="flex items-center justify-center p-8 sm:p-12 bg-base-100">
      <div class="w-full max-w-md">
        <div class="text-center mb-8 lg:hidden">
          <img src="/logo/tributee_combination-no-motto-white.png" alt="Tributee Logo" class="h-20 mx-auto">
        </div>
        <h2 class="text-3xl font-bold text-center mb-2">Create Your Account</h2>
        <p class="text-center text-base-content/70 mb-8">Let's get you started with a new account</p>
        <form @submit.prevent="handleSignup" class="space-y-4" novalidate>
          ...
          <div class="form-control">
            ...
            <input type="text" placeholder="John Doe" class="input input-bordered w-full" :class="{ 'input-error': nameError }" v-model="name" required :disabled="loading">
          ...
          <div class="form-control">
            ...
            <div class="relative">
              <input :type="showConfirmPassword ? 'text' : 'password'" placeholder="••••••••" class="input input-bordered w-full pr-10" :class="{ 'input-error': confirmPasswordError }" v-model="confirmPassword" required :disabled="loading">
              <button type="button" @click="showConfirmPassword = !showConfirmPassword" class="absolute inset-y-0 right-0 px-3 flex items-center text-gray-500" :disabled="loading">
        ...
        <div class="text-center mt-6">
          <p class="text-base-content/70">Already have an account? <router-link to="/login" class="link link-primary font-medium">Log in</router-link></p>
        </div>
      </div>
    </div>
  </div>
...
const carouselItems = ref([
  { title: 'Join Tributee Today!', text: 'Create an account to get started.' },
  { title: 'Unleash Your Creativity', text: 'Build beautiful tributes for the people you admire.' },
  { title: 'Connect with Others', text: 'Share your tributes and discover what inspires others.' },
])
